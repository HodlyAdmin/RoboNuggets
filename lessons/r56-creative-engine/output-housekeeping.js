...
function classifyManifest(manifest) {
  ...
  const stageResults = Object.values(stages).filter(Boolean);
  const completed = stageResults.filter(stage => stage?.status === 'completed').length;
  const failed = stageResults.filter(stage => stage?.status === 'failed').length;
  const pending = stageResults.filter(stage => String(stage?.status || '').includes('pending')).length;
  ...
  if (pending > 0 || stageResults.length > 0) {
    return 'incomplete';
  }
...
async function main() {
  ...
  log.header('R56 Output Housekeeping');
...
if (resolve(process.argv[1] || '') === __filename) {
  main().catch((error) => {
    log.error(`R56 output housekeeping failed: ${error.message}`);
    process.exit(1);
  });
}
