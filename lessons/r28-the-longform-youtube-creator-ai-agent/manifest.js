...
export async function saveManifest(data) {
  ...
  const manifest = {
    version: '1.0',
    lesson: 'R28 | R28 | The Longform YouTube Creator AI Agent',
    createdAt: new Date().toISOString(),
    config: { projectName: data.projectName },
    stages: {
      ideaGeneration: data.ideaGeneration || null,
      scriptAndPromptGen: data.scriptAndPromptGen || null,
      voiceoverGen: data.voiceoverGen || null,
      videoAssembly: data.videoAssembly || null,
      youtubePublishing: data.youtubePublishing || null,
    }
  };
