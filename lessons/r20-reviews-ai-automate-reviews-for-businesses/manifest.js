...
export async function saveManifest(data) {
  ...
  const manifest = {
    version: '1.0',
    lesson: 'R20 | R20 | Reviews AI: Automate reviews for businesses',
    createdAt: new Date().toISOString(),
    config: { projectName: data.projectName },
    stages: {
      fetchReviews: data.fetchReviews || null,
      generateResponse: data.generateResponse || null,
      publishReply: data.publishReply || null,
    }
  };
