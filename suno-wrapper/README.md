...
> Local fork note
> In this RoboNuggets repo, the wrapper is being used as a local hybrid API standard. Although the upstream README documents a `2Captcha` path, this fork also contains a no-paid-solver Wingman/manual fallback in [src/lib/SunoApi.ts](./src/lib/SunoApi.ts): when Suno or Google blocks automation, the wrapper can foreground the browser, wait for manual resolution, intercept the token, and resume.
> The local fork also supports `BROWSER_KEEP_OPEN=true` for debugging native browser runs. Leave it `false` for normal operation so the persistent Suno profile is released cleanly after each run.
...
The upstream project documents a paid [2Captcha](https://2captcha.com/about) service path (a.k.a. ruCaptcha) for automated CAPTCHA solving. In this local RoboNuggets fork, that path is optional: the preferred runtime is the zero-paid-solver Wingman/manual fallback, with `2Captcha` retained only as an optional automation strategy.
...
- Supports both paid [2Captcha](https://2captcha.com) automation and the local no-paid-solver Wingman/manual fallback built on [Playwright](https://playwright.dev) with [rebrowser-patches](https://github.com/rebrowser/rebrowser-patches).
...
### 2. Optional: register on 2Captcha and top up your balance
[2Captcha](https://2captcha.com/about) is a paid CAPTCHA solving service that uses real workers to solve the CAPTCHA and has high accuracy. In the upstream project it is the primary CAPTCHA strategy. In this local fork it is optional, because the wrapper can also pause, foreground the browser, and let a human resolve the Suno/Google challenge manually.
...
- `TWOCAPTCHA_KEY` — optional 2Captcha API key if you want automated CAPTCHA solving instead of Wingman/manual fallback.
...
- `BROWSER_LOCALE` — the language of the browser. Using either `en` or `ru` is still a good default. If you use 2Captcha, those languages also have the deepest worker pool. [List of supported languages](https://2captcha.com/2captcha-api#language)
- `BROWSER_HEADLESS` — run the browser without the window. You probably want to set this to `true`.
- `BROWSER_KEEP_OPEN` — local fork option for debugging native Suno generation. When `true`, the browser stays open after submission so you can inspect the page manually. Keep this `false` during normal runs, otherwise the `.suno-browser-profile` directory stays locked until you close the browser yourself.
```bash
SUNO_COOKIE=<…>
TWOCAPTCHA_KEY=<…>
BROWSER=chromium
